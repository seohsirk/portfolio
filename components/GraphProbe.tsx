...
type Probe = { id: string; label: string; nodes: string[]; hops: number };
...
const NODES: { id: string; x: number; y: number; label: string }[] = [
  ...
  { id: "tech", x: 92, y: 55, label: "Technology" },
...
const PROBES: Probe[] = [
  { id: "match", label: "추천 후보 뽑기", nodes: ["co", "pain", "sol"], hops: 1 },
  { id: "industry", label: "회사–산업 매칭", nodes: ["co", "ind", "seg"], hops: 1 },
  { id: "rival", label: "유사 경쟁사", nodes: ["co", "tech"], hops: 1 },
];
  ...
  return (
    <figure className="probe">
      <div className="probe-btns">
        {PROBES.map((p) => (
          <button
            ...
            className={`probe-btn mono ${active?.id === p.id ? "is-on" : ""}`}
          >
            {p.label}
          </button>
        ))}
      ...
      <figcaption className="probe-cap mono">
        {active
          ? `${active.label} — 노드 ${active.nodes.length}개 · 최대 홉 ${active.hops}`
          : "세 개를 눌러 보면 다중 홉 탐색이 한 번도 일어나지 않는다"}
      </figcaption>
    </figure>
  );
}
